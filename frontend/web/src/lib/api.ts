...
export const chatApi = {
  getConversations: (c?: string) => api.get('/conversations', { params: { cursor: c, limit: 20 } }),
...
export const kbApi = {
  ...
  create: (d: any) => api.post('/knowledge-bases', d),
  get: (id: string) => api.get('/knowledge-bases/' + id),
  delete: (id: string) => api.delete('/knowledge-bases/' + id),
  listDocs: (id: string) => api.get('/knowledge-bases/' + id + '/documents'),
  uploadDoc: (id: string, f: File) => { const fd = new FormData(); fd.append('file', f); return api.post('/knowledge-bases/' + id + '/documents', fd); },
  deleteDoc: (kb: string, doc: string) => api.delete('/knowledge-bases/' + kb + '/documents/' + doc),
  search: (id: string, q: string) => api.post('/knowledge-bases/' + id + '/search', { query: q }),
  chat: (id: string, q: string) => api.post('/knowledge-bases/' + id + '/chat', { query: q }),
  ingestUrl: (id: string, url: string) => api.post('/knowledge-bases/' + id + '/ingest-url', { url }),
};
export const workflowsApi = {
  list: () => api.get('/workflows'),
  create: (d: any) => api.post('/workflows', d),
  get: (id: string) => api.get('/workflows/' + id),
  update: (id: string, d: any) => api.patch('/workflows/' + id, d),
  delete: (id: string) => api.delete('/workflows/' + id),
  execute: (id: string, d?: any) => api.post('/workflows/' + id + '/execute', d),
  getRuns: (id: string) => api.get('/workflows/' + id + '/runs'),
};
export const marketplaceApi = {
  list: (p?: any) => api.get('/marketplace', { params: p }),
  get: (id: string) => api.get('/marketplace/' + id),
  create: (d: any) => api.post('/marketplace', d),
