...
export function ChatClient() {
  ...
  const newConv = async () => { try { const { data } = await chatApi.createConversation({ title: 'New Chat', model: selectedModels[0] }); addConversation(data.data); setActiveConversation(data.data.id); setMessages(data.data.id, []); } catch {} };
