    ...
    <div className="flex h-full">
      <div className="w-56 border-r border-border flex flex-col shrink-0 bg-card">
        ...
        <div className="flex-1 overflow-y-auto py-2 px-2 space-y-1">
          {conversations.map(c=><button key={c.id} onClick={()=>selectConv(c.id)} className={"w-full text-left px-3 py-2 rounded-lg text-sm truncate "+(c.id===activeConversationId?'bg-accent text-foreground':'text-muted-foreground hover:bg-accent/50')}>💬 {c.title}</button>)}
          {conversations.length===0&&<p className="text-xs text-muted-foreground px-3 py-4 text-center">No conversations yet</p>}
        </div>
      </div>
      <div className="flex-1 flex flex-col min-w-0">
        <div className="border-b border-border px-4 py-2 flex items-center gap-3 flex-wrap bg-card/50">
          <div className="flex rounded-lg border border-border overflow-hidden">
            {(['single','compare','router'] as const).map(m=><button key={m} onClick={()=>setChatMode(m)} className={"px-3 py-1.5 text-xs font-medium "+(chatMode===m?'bg-primary text-primary-foreground':'text-muted-foreground hover:text-foreground')}>{m}</button>)}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
            ...
            {isStreaming&&streamingContent&&<div className="flex gap-3"><div className="w-8 h-8 rounded-full bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center text-xs text-white font-bold shrink-0">AI</div><div className="flex-1 bg-card rounded-2xl rounded-tl-sm px-4 py-3 text-sm border border-border whitespace-pre-wrap">{streamingContent}<span className="inline-block w-0.5 h-4 bg-violet-500 animate-pulse ml-0.5"/></div></div>}
