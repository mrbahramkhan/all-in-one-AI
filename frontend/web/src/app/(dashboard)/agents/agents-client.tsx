  ...
  return(
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {agents.map((a:any)=><div key={a.id} className="p-4 rounded-xl border border-border bg-card hover:border-violet-500/50 transition-colors"><div className="flex items-center gap-3 mb-3"><div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-cyan-500 flex items-center justify-center text-white font-bold">{a.name?.[0]||'A'}</div><div><p className="font-semibold text-sm">{a.name}</p><p className="text-xs text-muted-foreground">{a.model}</p></div></div>{a.description&&<p className="text-xs text-muted-foreground line-clamp-2 mb-3">{a.description}</p>}<button className="w-full py-1.5 text-xs bg-violet-600 hover:bg-violet-700 text-white rounded-lg">Chat</button></div>)}
        {agents.length===0&&<div className="col-span-3 text-center py-16 text-muted-foreground"><div className="text-4xl mb-3">🤖</div><p className="font-medium">No agents yet</p><p className="text-sm mt-1">Create your first AI agent!</p></div>}
      </div>
